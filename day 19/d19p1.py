# we're gonna do this by imagining the beacons as vertices of a 3d shape, caring only about
# edge magnitudes of the edges. compare edge lengths to find common beacons.
# this is long code: the bulk of it is to put the data into nice forms

# My method is frankly, very long but ultimately going through classic linear algebra principles.
# This almost feels more like a data science reporting project: quite often, it's viable to precalculate some parts
# and use the results in memory; I'll leave the code in used for this but will keep it commented out

# A Jupyter notebook would actually be a really neat way of doing this.

# finds magnitude cubed. No point in rooting, as jsut focused on relatives not absolutes
def magnitude(a,b):
    mag=0
    for i in range(3):
        mag+=(a[i]-b[i])**2
...
for i in range(len(magvals)):
    for j in range(i+1,len(magvals)):
        ...
        for x in magvals[i]:
            ...
            if added:
                break
# block above is used to get sharescans. Can use to save time, or as a note to think about.
# if samevals(magvals[i][x], magvals[j][y])==12, means x and y are the same point, but in i/j coordinate systems.
        ...
        for y in range(len(magvals[i[1]])):
            if same_vals(magvals[i[0]][x],magvals[i[1]][y])>11:
                ...
                for z in range(len(magvals[i[0]][x])):
                    ...
                    # this prevents the 'check this out' error in line 38
...
# add all absolute values to map (a set here, so duplicated can't exist)
map = set() # will store points in a neat, efficient manner with no need to worry about duplicates
# throw in points for sensor 0, as it's the reference
for i in absolutepoints:
    for j in absolutepoints[i]:
        map.add(tuple(j))
