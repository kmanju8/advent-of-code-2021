...
while line:
    ...
    snailnos.append(eval(line))
    line = file.readline().strip()

maximum=0
for i in range(len(snailnos)):
    for j in range(len(snailnos)):
        if i==j:
            continue
        
        current_tree = add_nodes(to_tree(snailnos[i]),to_tree(snailnos[j]))
        ...
        maximum=max(maximum,magnitude(current_tree))
